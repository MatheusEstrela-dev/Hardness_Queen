...
**Cabimento do modelo em 4-bit nos 8GB da T1000 -- medido, com ressalva sobre qual variante.** O plano previa medir Qwen2.5-7B-Instruct. Na pratica, o link desta estacao entrega a CDN da Hugging Face a cerca de 1,71 MB/s agregado (medido via multiplos fragmentos em paralelo), o que estima ~2h para os ~15GB de pesos do 7B-Instruct -- tempo que nao coube na sessao. A medicao registrada abaixo usa **Qwen2.5-3B (variante base, sem instruction tuning)**, ja presente em disco por causa do treino de `scripts/02_treinar_modelo.py`:

- VRAM de pico: **2,00 GB** (`torch.cuda.max_memory_allocated`), bem dentro dos 8 GB da T1000. Este numero **inclui** o carregamento do modelo (pesos 4-bit entrando na GPU) -- e a resposta a pergunta "cabe?".
- Tempo por chunk: **62,8s** para uma unica extracao (chunk curto, ~2 limiares), incluindo warmup do `outlines_core` -- o log mostrou avisos `WON'T CONVERT` porque o Triton nao esta instalado nesta maquina Windows, entao o `torch.compile` do kernel de bitmask cai para modo eager; chamadas subsequentes na mesma sessao devem ser mais rapidas, mas isso nao foi medido. Este numero **exclui** o carregamento do modelo -- o cronometro so comeca depois que `criar_gerador_qwen()` retorna -- porque e a resposta a pergunta "quanto custa cada chunk adicional", relevante para um `processar()` que carrega o gerador uma vez e roda em loop sobre muitos chunks.
- Qualidade de extracao: o trecho de teste com dois limiares (80mm e 100mm) produziu as duas regras, ambas com `status="ok"`. A decodificacao restrita do `outlines` garante validade estrutural do JSON independente da variante do modelo (nao e comportamento aprendido); o que a variante base poderia faltar e o instruction-following para notar que o trecho cita dois limiares -- e mesmo assim ela notou. Este numero e um **piso**, nao o resultado esperado do 7B-Instruct: e razoavel *esperar* que a variante Instruct mantenha ou melhore essa taxa de extracao, mas isso nao foi medido -- e uma inferencia sobre modelos com instruction tuning, nao um fato estabelecido por este teste.
- Download do 7B-Instruct: iniciado e nao interrompido nesta sessao, ficou em progresso para uma medicao complementar em sessao futura -- o numero acima nao substitui essa medicao, apenas estabelece que a infraestrutura (VRAM, decodificacao restrita, pipeline) funciona de ponta a ponta.
- Achado de infraestrutura: a primeira execucao real de `criar_gerador_qwen` (nunca antes executada, so validada por inspecao de assinatura) revelou que `outlines` 1.3.3 repassa `seed` como kwarg de inferencia direto para `transformers.generate()`, que rejeita chaves nao reconhecidas (`ValueError: model_kwargs nao usado: ['seed']`). Corrigido substituindo por `torch.manual_seed(SEMENTE)` na criacao do gerador; a decodificacao continua determinista via `temperature=0` (gulosa).
