...
- VRAM de pico: **2,00 GB** (`torch.cuda.max_memory_allocated`), bem dentro dos 8 GB da T1000.
...
- Qualidade de extracao: o trecho de teste com dois limiares (80mm e 100mm) produziu as duas regras, ambas com `status="ok"`. A decodificacao restrita do `outlines` garante validade estrutural do JSON independente da variante do modelo (nao e comportamento aprendido); o que a variante base poderia faltar e o instruction-following para notar que o trecho cita dois limiares -- e mesmo assim ela notou. Este numero e um **piso**, nao o resultado esperado do 7B-Instruct: o Instruct deve manter ou melhorar a taxa de extracao, nunca piorar por essa medicao.
